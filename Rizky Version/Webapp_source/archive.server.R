...
function(input, output, session) {
  ...
  seuratData <- reactive({
    ...
    if (!is.na(rds_file)) {
      ...
      updateSelectizeInput(session = session,inputId = 'GeneInput',choices=rownames(seurat_obj),selected = NULL,server = T)
  ...
  observeEvent(input$plotButton_UMAP, {
    ...
    if (!is.null(input$GeneInput)) {
      feature_plot <- FeaturePlot(object = seurat_data, features = GeneInput,reduction = "umap")
      print()
    }
